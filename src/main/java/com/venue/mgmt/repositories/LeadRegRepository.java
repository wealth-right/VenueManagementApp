...
public interface LeadRegRepository extends JpaRepository<LeadRegistration, Long> {
    ...
    @Query("SELECT l FROM LeadRegistration l WHERE l.createdBy = :userId and l.isDeleted = false order by l.creationDate desc")
    ...
    Optional<LeadRegistration> findByEmail(String email);

    int countByVenue_VenueIdAndCreatedByAndCreationDateBetweenAndIsDeletedFalse(Long venueId, String userId, Date startOfDay,Date endOfDay);
}
